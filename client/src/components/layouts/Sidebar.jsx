...
const navItems = [
  { label: 'Ask AyurAI', icon: MessageSquare },
  { label: 'Classical Texts', icon: BookOpen },
  { label: 'Plant ID', icon: Leaf },
  { label: 'IP Guidance', icon: Scale },
]

const Sidebar = () => {
  const [active, setActive] = useState('Ask AyurAI')
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col border-r border-gold/10 bg-forest/60 p-4">
      ...
      <button className="mb-6 flex w-full items-center justify-center gap-2 rounded-xl bg-gold py-2.5 font-semibold text-deep transition hover:opacity-90">
        <Plus size={18} />
        New chat
      </button>
